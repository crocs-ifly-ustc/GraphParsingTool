...
def convert_graph_to_json(root_atom):
    out_dict = {}
    all_atoms = GetAllAtoms(root_atom)
    SimulateCoord(root_atom)
    
    out_dict["atoms"] = {}
    out_dict["bonds"] = {}
    for atom in all_atoms:
        out_dict["atoms"][atom.name] = atom.format_to_json()
        for bond in atom.in_bonds + atom.out_bonds:
            bond_key = bond.begin_atom.name + "-" + bond.end_atom.name
            if bond_key not in out_dict["bonds"]:
                out_dict["bonds"][bond_key] = bond.format_to_json()
            else:
                format_str = bond.format_to_json()
                if format_str != out_dict["bonds"][bond_key]:
                    pdb.set_trace()
